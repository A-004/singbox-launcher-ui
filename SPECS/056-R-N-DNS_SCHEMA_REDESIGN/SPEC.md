...
дев-маркер до релиза — на релизе bump будет с уже-правильным дизайном):
...
1. `state.json` после Save имеет секцию `dns_options` с скалярами
   (strategy / final / independent_cache / default_domain_resolver) и
   `servers[]`/`rules[]` через `kind`. Секции `dns.template_servers`,
   `dns.extra_servers`, `dns.extra_rules` отсутствуют.
2. `vars[]` НЕ содержит `dns_*` элементов (откат SPEC 032 для DNS scalars).
3. **Удалены** функции:
...
6. **Dev state переписывается на месте.** Schema version и name НЕ меняются
   ...
   (`dns.template_servers`/`extra_servers`/`extra_rules` + `vars[dns_*]`),
   write в новом shape, backup `state.json.dev-pre-redesign.bak`.
...
1. **Phase 1 — Schema:** новый `v6.DNSOptions` struct с `Strategy/Final/...` +
   `Servers []DNSServer{Kind, Tag, Enabled, Body}` + `Rules []DNSRule{Kind, Body}`.
   Schema version и name **не меняются** — это переписывание дев-формата
   v6/`presets_v1` на месте. Старый `v6.DNSConfig` удаляется (template_servers/
   extra_servers/extra_rules).
...
2. **Phase 2 — In-place dev rewrite:** на parseV6 — если встречаем старый
   ...
   / `vars[dns_*]`), читаем по-старому, конвертим в in-memory `v6.DNSOptions`
   нового shape, backup `state.json.dev-pre-redesign.bak`, на ближайшем
   Save файл перезаписывается в новом layout'е.
