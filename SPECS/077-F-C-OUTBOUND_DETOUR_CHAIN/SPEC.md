...
- **Сужение целей в UI (уточнено после ревью).** Дропдаун (`business.DetourOptions`) предлагает **только ручные глобальные selector-группы** (созданные на вкладке Outbounds) и **активные preset-группы**. Намеренно исключены: встроенные/служебные `direct-out`/`reject`/`drop` (хоп через них бессмыслен), auto-группа `auto-proxy-out` (urltest-автовыбор), **локальные авто/select-группы самих подписок** (служебные группы над подпиской, не хоп), отдельные узлы подписок (нестабильные runtime-теги) и **одиночные серверы пока** (их теги тоже runtime-only; добавление в `GetAvailableOutbounds` протекло бы в выбор target'а правил — отложенный п.2 обсуждения). Реестр `GetAvailableOutbounds` при этом **не трогается** — фильтрация живёт только в `DetourOptions`.
